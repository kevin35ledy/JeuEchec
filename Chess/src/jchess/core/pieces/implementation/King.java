...
public class King extends Piece
{
	...
	public boolean isSafe(Square s)
	{
		...
		for(int i=0; i<squares.length; i++)
		{
			for(int j=0; j<squares[i].length; j++)
			{
				...
				if(piece != null) 
				{
					//si ennemi ou soi-meme
					if(piece.getPlayer().getColor() != this.getPlayer().getColor() )
					{

						//si piece est un pion le roi peut aller juste devant lui en étant safe
						//donc on différencie le cas piece -> pion et piece != pion
	...
	public boolean willBeSafeAfterMove(Square currentSquare, Square futureSquare)
	{
		...
		boolean ret;
		if (futureSquare.piece.equals(this)){
			ret = isSafe(futureSquare); //si on déplace le roi vérifier qu'il ne se met pas en danger tout seul
		}
		else{//si piece différente du roi vérifier qu'on ne met pas en danger le roi
