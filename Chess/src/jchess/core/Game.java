 ...
 * @author: Damian Marciniak Class responsible for the starts of new games,
 *          loading games, saving it, and for ending it. This class is also
 *          responsible for appoing player with have a move at the moment
 */
public class Game extends JPanel implements ComponentListener, MouseListener {
	...
	public void nextMove() {
		switchActive();
		LOG.debug("next move, active player: " + activePlayer.getName() + ", color: " + activePlayer.getColor().name()
		...
		} else if (event.getButton() == MouseEvent.BUTTON1) // left button
		{
			...
			if (!isChessboardBlocked()) {
				...
				try {
					...
					if (sq.piece != null && sq.getPiece().getPlayer() == this.activePlayer
					...
					{
						getChessboard().unselect();
					} else if (getChessboard().getActiveSquare() != null
							&& getChessboard().getActiveSquare().piece != null
